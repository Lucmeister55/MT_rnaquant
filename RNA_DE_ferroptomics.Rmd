...
for (i in seq_along(dds_deseq_list)) {
  ...
  resultsNames(dds)
  res_RvsS <- results(dds, independentFiltering=TRUE, contrast=c("Group","R","S")) #Last level is the base level
  summary(subset(res_RvsS, padj < 0.05))
  
  res_RvsS_lfc <- lfcShrink(dds, contrast = c("Group", "R", "S"), type = "ashr", lfcThreshold = 0, res = res_RvsS)
  
  summary(subset(res_RvsS_lfc, padj < 0.05))
