...
pca_full_ntop500 <- pcaplot(rld_full, intgroup=c("tumor_type", "Group"), ellipse = TRUE, text_labels = TRUE)
pca_full_ntop500
dev.off()

png(paste0(dir, "/PCA_NB_ntop500.png"), width = 900, height = 900)
pca_NB_ntop500 <- pcaplot(rld_NB, intgroup=c("tumor_type", "Group"), ellipse = TRUE, text_labels = TRUE)
pca_NB_ntop500
dev.off()

png(paste0(dir, "/PCA_MM_ntop500.png"), width = 900, height = 900)
pca_MM_ntop500 <- pcaplot(rld_MM, intgroup=c("tumor_type", "Group"), ellipse = TRUE, text_labels = TRUE)
pca_MM_ntop500
dev.off()

png(paste0(dir, "/PCA_sensSHY_ntop500.png"), width = 900, height = 900)
pca_sensSHY_ntop500 <- pcaplot(rld_sensSHY, intgroup=c("tumor_type", "Group"), ellipse = TRUE, text_labels = TRUE)
pca_sensSHY_ntop500
dev.off()
...
for (i in seq_along(resdata_annot_list)) {
  ...
  resdata_annot <- resdata_annot_list[[i]] %>%
    filter(padj < 0.05)
  gene_sets[[title]] <- unique(na.omit(resdata_annot$name))
}

whitelist = read.csv("complete_gene_list_no_duplicates.txt")
gene_sets[["whitelist"]] = whitelist[, 2]
...
upset_plot <- upset(upset_data, 
                    ...
                    keep.order = TRUE, 
                    mainbar.y.label = "Gene Symbol Intersections")

print(upset_plot)

# Save the plot as a PNG file with increased resolution
png("outputs/DifferentialAnalysis/upset_plot.png", width = 1600, height = 1000, pointsize = 10)
print(upset_plot)
dev.off()
```
