...
pcaplot(rld_sensSHY, intgroup=c("Group"), ellipse = FALSE)
dev.off()
...
pcaplot(rld_sensSHY, intgroup=c("Group"), ellipse = FALSE, ntop = 10000)
dev.off()
```
...
for (i in seq_along(resdata_annot_list)) {
  ...
  resdata_annot <- resdata_annot_list[[i]] %>%
    filter(padj < 0.05)
  gene_sets[[title]] <- na.omit(resdata_annot$name)
}
...
upset_plot <- upset(upset_data, sets = names(gene_sets), keep.order = TRUE)
