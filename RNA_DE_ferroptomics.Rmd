...
dds_full <- DESeqDataSetFromMatrix(countdata_fc_full, sampleTable_exp, design = ~ gender + Group)
...
png(paste0(dir, "/PCA_filt1_ntop500.png"), width = 900, height = 900)
pcaplot(rld_filt1, intgroup=c("Group"), ellipse = FALSE)
dev.off()

png(paste0(dir, "/PCA_filt2_ntop500.png"), width = 900, height = 900)
pcaplot(rld_filt2, intgroup=c("Group"), ellipse = FALSE)
dev.off()

png(paste0(dir, "/PCA_filt3_ntop500.png"), width = 900, height = 900)
...
png(paste0(dir, "/PCA_filt1_ntop10000.png"), width = 900, height = 900)
pcaplot(rld_filt1, intgroup=c("Group"), ellipse = FALSE, ntop = 10000)
dev.off()

png(paste0(dir, "/PCA_filt2_ntop10000.png"), width = 900, height = 900)
pcaplot(rld_filt2, intgroup=c("Group"), ellipse = FALSE, ntop = 10000)
dev.off()

png(paste0(dir, "/PCA_filt3_ntop10000.png"), width = 900, height = 900)
pcaplot(rld_filt3, intgroup=c("Group"), ellipse = FALSE, ntop = 10000)
dev.off()
```
```{r}
...
for (i in seq_along(dds_list)) {
  ...
  design(dds) <- ~ gender + Group 

  dds_deseq <- DESeq(dds)
  
  res <- results(dds_deseq, alpha = 0.05, lfcThreshold = 2)
  summary(res)
...
for (i in seq_along(dds_deseq_list)) {
  ...
  res_RvsS_full <- results(dds, independentFiltering=TRUE, contrast=c("Group","R","S"))
  
  sink(paste0("rnaseq_ferroptomics/DifferentialAnalysis/results_summary_", affix, ".txt"))
  summary(res_RvsS)
  sink()
  
  res_RvsS_lfc <- lfcShrink(dds, contrast = c("Group", "R", "S"), type = "ashr", alpha = 0.05, res = res_RvsS)
  res_RvsS_lfc_full <- lfcShrink(dds, contrast = c("Group", "R", "S"), type = "ashr", res = res_RvsS_full)
  
  sink(paste0("rnaseq_ferroptomics/DifferentialAnalysis/resultsLFC_summary_", affix, ".txt"))
  summary(res_RvsS_lfc)
  sink()
  
  res_RvsS_lfc_list <- c(res_RvsS_lfc_list, res_RvsS_lfc_full)
}
...
```
```{r}
res_RvsS <- results(dds, independentFiltering=TRUE, contrast=c("Group","R","S"), alpha = 0.05, lfcThreshold = 2)
res_RvsS_lfc <- lfcShrink(dds, contrast = c("Group", "R", "S"), type = "ashr", alpha = 0.05, lfcThreshold = 2, res = res_RvsS)
res <- res_RvsS_lfc
summary(res)
```
