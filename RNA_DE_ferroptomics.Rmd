...
```{r}
new_ids <- sampleTable_exp$sample_id_adj[match(colnames(countdata_fc), sampleTable_exp$run_id)]
colnames(countdata_fc) <- new_ids
countdata_fc <- countdata_fc[, sampleTable_exp$sample_id_adj]
head(countdata_fc)

# names(countdata_fc) == sampleTable_exp$sample_id_adj

dds <- DESeqDataSetFromMatrix(countdata_fc, sampleTable_exp, ~1)
dds$Group <- relevel(dds$Group, "S")
...
Comparing two types of samples, wich differ by a single covariate (here: `Group`), ("a two-group comparison") is  straight-forward.
...
res_RvsS_lfc <- lfcShrink(dds, contrast = c("Group", "R", "S"), type = "ashr", lfcThreshold = 0, res = res)
res_RvsS_lfc <- subset(res_RvsS_lfc, padj < 0.05)
...
plotMA(res_RvsS_lfc, ylim=c(-4,10))
dev.off()
```
...
resdata <- merge(as.data.frame(res_RvsS_lfc), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
...
```{r}
cols <- c("GENENAME", "ENTREZID", "SYMBOL")
annotdata <- AnnotationDbi::select(org.Hs.eg.db, keys=countdata_fc_raw$Ensembl_ID, columns=cols, keytype="ENSEMBL")

#Check for non-unique ensg and remove duplicates if necessary
n_occur <- data.frame(table(annotdata$ENSEMBL))
n_occur[n_occur$Freq > 1,]
annotdata <- annotdata[!(annotdata$ENSEMBL %in% n_occur$Var1[n_occur$Freq > 1]),]
# Manually erase from annotation file where necessary

rownames(annotdata) <- annotdata$ENSEMBL
...
png(paste("rnaseq_ferroptomics/DifferentialAnalysis/genes_heatmap_",celline,"_",contrast,".png", sep=""), 1200, 1000, pointsize=20)
